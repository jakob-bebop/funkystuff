%% version Y/M/D = 2014/08/10 : for lilypond 2.18 or higher
...
"Make a list of lists of moments with
  ...
  2nd element = length, between this time-signature and the following
           ...
           (cond
             ((eq? name 'TimeSignatureMusic);; a time signature change occurs
                (let ((measure-len (ly:make-moment 
                  ...
                  (if (ly:moment<? ZERO-MOMENT local-pos) ; if not local-pos = 0
                    (add-info prev-measure-len local-pos local-pos)) ; add an info entry
                  (set! prev-measure-len measure-len)
                  (set! local-pos ZERO-MOMENT)))          ; reset for the next section
             
             ;; 'partial-duration has been replaced by 'duration in 2.17
             ;; https://codereview.appspot.com/12096043/
             ((or dur (eq? name 'EventChord))
                (if (eq? name 'PartialSet)   ;; for version >= 2.17.?
                  (let ((offset (ly:duration-length dur)))
                    (add-info prev-measure-len ZERO-MOMENT offset)
                    (set! local-pos (ly:moment-sub ZERO-MOMENT offset)))
                  (set! local-pos (ly:moment-add local-pos ;; (for all versions)
                                               (ly:music-length evt)))))
;           ((eq? name 'PartialSet) ;; \partial  ;; only for version < 2.17.?
...
;                   (set! local-pos (ly:moment-sub ZERO-MOMENT offset))))
             ...
             ((and (eq? name 'PropertySet) ;; \set Timing.measurePosition
                ...
                (let* ((offset (ly:music-property evt 'value)) ; offset < 0
                         ;; local-pos = n * 1-measure-len + remainder
                       (remain (ly:moment-mod local-pos prev-measure-len))) ; remainder
                      ; (corrected-pos (ly:moment-add 
                      ;                  local-pos 
                      ;                   (ly:moment-sub prev-measure-len offset))))
                ;;(format #t "~a - ~a - ~a\n" offset remain corrected-pos)
                (if (ly:moment<? ZERO-MOMENT remain)
                  (let*((pos1 (ly:moment-sub local-pos remain))
                        (pos2 (ly:moment-sub ZERO-MOMENT offset))
                        (pos3 (ly:moment-sub remain offset))
                        (corrected-pos (ly:moment-add local-pos pos2)))
                    (add-info prev-measure-len pos1 pos1)
                    (add-info pos2 pos2 pos3)
                    (set! local-pos corrected-pos))
                  (let ((pos1 (ly:moment-sub local-pos prev-measure-len))
                        (pos2 (ly:moment-sub prev-measure-len offset))
                        (corrected-pos (ly:moment-add local-pos offset)))
                    (add-info prev-measure-len pos1 pos1)
                    (add-info pos2 pos2 pos2)
                    (set! local-pos corrected-pos)))))
             (else
     ...
     (reverse  ; revert the list in the right order, but before, add the timing info of
               ; the last section. This section can be incomplete in the last measure.
       (let* ((mod (ly:moment-mod local-pos prev-measure-len)) ; the remaining length
              (nlength (ly:moment-sub local-pos mod)))         ; the exact section
         ;(format #t "mod=~a | nlength=~a | local-pos=~a\n" mod nlength local-pos)
         (if (ly:moment<? ZERO-MOMENT nlength)            ; don't add negative length
             (add-info prev-measure-len nlength nlength))
         (if (eq? mod ZERO-MOMENT)
            signature-list	      ; no remains length
            (cons (list mod mod mod)  ; like 1 measure in a "mod" signature
                  signature-list)))))))
 ...
 (let loop ((pointer-list signature-list))
   (if (pair? pointer-list)               ; each elt of signature-list is a
       (let* ((info (car pointer-list))   ; list (info) describing each sections.
              ...
              (raw-len (third info))        ; idem but ignores manual timing tweaks
              (mom (ly:moment-div nmeasures-len 1measure-len)) ; the denominator
                ; should be always = 1, but assume an error somewhere ...
              (n (quotient (ly:moment-main-numerator mom)     ; n = number of measures
           ...
           (set! current-moment (ly:moment-add current-moment raw-len))
           ;(format #t "mom : ~a | n : ~a | current-moment : ~a\n" mom n current-moment)
           (cond
